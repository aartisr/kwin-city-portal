...
| Evidence ledger | Supabase migrations `0003`–`0006` | Append-only evidence, integrity enforcement, and atomic idempotent recording |
...
The first successful run verifies that GitHub can execute the controls and persist two signed, policy-qualified evidence records without exposing secrets in logs. Apply migrations `0003` through `0006` before this run.
