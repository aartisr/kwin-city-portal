...
const TOOL_STARTERS = [
  {
    ...
    href: "/tools/risk-check",
    icon: "🛡️",
    prompt: "Can this site, route, or plan withstand scrutiny?",
    action: "Assess risk",
  },
  {
    ...
    href: "/tools/valuation-index",
    icon: "📊",
    prompt: "Where are value, demand, and opportunity signals moving?",
    action: "Read the market",
  },
  {
    lane: "Spatial and Progress",
    href: "/tools/spatial-explorer",
    icon: "🗺️",
    prompt: "What is changing on the ground, and where?",
    action: "Explore the map",
  },
  {
    lane: "Policy and Data",
    href: "/tools/regulatory-navigator",
    icon: "📋",
    prompt: "What rules, records, or datasets should I check next?",
...
export default async function ToolsIndexPage() {
  const locale = await getServerLocale();
  const l = (values: Parameters<typeof pickByLocale<string>>[1]) =>
    pickByLocale(locale, values);
  const quickStartRoutes = TOOL_ROUTES.filter((tool) =>
    QUICK_START_HREFS.includes(tool.href as (typeof QUICK_START_HREFS)[number]),
  );
      ...
      <main id="main-content" role="main">
        ...
        <section className="section bg-slate-50">
          <div className="container">
            ...
            <nav
              aria-label="Choose a tool by task"
              className="mb-5 rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white shadow-[0_18px_46px_rgba(15,23,42,0.14)]"
            >
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {TOOL_STARTERS.map((starter) => (
                  <Link
                    key={starter.lane}
                    href={starter.href}
                    className="group rounded-xl border border-white/15 bg-white/[0.06] p-4 transition hover:-translate-y-0.5 hover:border-amber-200 hover:bg-white/[0.1]"
                  >
                    ...
                    <span className="mt-3 inline-flex text-sm font-bold text-amber-200 group-hover:text-amber-100">
                      {starter.action} →
                    </span>
                  </Link>
                ))}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-extrabold text-slate-900">
                    {l({
                      en: "Start with the strongest decision paths",
                      kn: "ಬಲವಾದ ನಿರ್ಧಾರ ಮಾರ್ಗಗಳಿಂದ ಪ್ರಾರಂಭಿಸಿ",
                      hi: "सबसे मजबूत निर्णय मार्गों से शुरू करें",
                    })}
                  </h2>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-[#6F3F00]">
                    {l({
                      en: "Shift+T or Cmd/Ctrl+K",
                      kn: "Shift+T ಅಥವಾ Cmd/Ctrl+K",
                      hi: "Shift+T या Cmd/Ctrl+K",
                    })}
                  </span>
                  ...
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
                    {l({
                      en: "4 intent lanes",
                      kn: "4 ಉದ್ದೇಶ ಮಾರ್ಗಗಳು",
                      hi: "4 इंटेंट लेन",
                    })}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
                    {l({
                      en: "Source-linked outputs",
                      kn: "ಮೂಲ-ಲಿಂಕ್ಡ್ ಔಟ್‌ಪುಟ್‌ಗಳು",
                      hi: "स्रोत-लिंक्ड आउटपुट",
                    })}
                  </span>
                </div>
              </div>
