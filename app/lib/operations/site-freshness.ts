...
function readDatedField(path: string, matcher: RegExp, fallbackDate: string): FreshnessRecord {
  try {
    ...
    console.warn(`Using embedded freshness baseline for ${path}: ${error instanceof Error ? error.message : String(error)}`);
