...
export function parseVerificationSubmission(value: unknown): VerificationSubmission {
  ...
  if (value.providerRunUrl != null && (typeof value.providerRunUrl !== 'string' || !HTTPS.test(value.providerRunUrl))) throw new Error('invalid-provider-url');
  ...
  if (!Array.isArray(value.controls) || value.controls.length > 100) throw new Error('invalid-controls');
  const controls = value.controls.map((control) => {
    ...
    return {
      ...
      metrics: isObject(control.metrics) ? control.metrics as VerificationSubmission['controls'][number]['metrics'] : undefined,
    };
  });
  return {
    ...
    providerRunUrl: value.providerRunUrl as string | undefined,
