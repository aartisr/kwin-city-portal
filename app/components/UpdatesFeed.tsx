...
const TIER_CONFIG: Record<VerificationTier, { label: string; icon: string; badge: string }> = {
  verified:   { label: 'Verified',   icon: '✅', badge: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
  pending:    { label: 'Pending',    icon: '🔍', badge: 'text-amber-700 bg-amber-50 border-amber-200' },
  contextual: { label: 'Contextual', icon: '⚪', badge: 'text-gray-600 bg-gray-100 border-gray-200' },
};
...
function UpdateCard({ entry, catMeta }: { entry: UpdateEntry; catMeta: UpdateCategory | undefined }) {
  const [expanded, setExpanded] = useState(false);
  const tier = TIER_CONFIG[entry.verificationTier];
    ...
    <main className="min-h-screen bg-gradient-to-br from-white via-slate-50/30 to-white">
      ...
      <section className="pt-28 pb-12 border-b border-gray-100">
        <div className="container">
          <p className="text-xs font-bold tracking-[0.2em] uppercase text-blue-600 mb-4">Project Record</p>
        ...
        <div className="container">
          <div className="max-w-3xl">
            ...
            <div className="relative pl-8">
                ...
                {filtered.map((entry, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <UpdateCard entry={entry} catMeta={catMeta} />
                    </motion.div>
                  );
                })}
              </AnimatePresence>
              ...
              {filtered.length === 0 && (
                <div className="text-center py-16 -ml-8">
                  ...
                  <p className="text-gray-500 text-sm">No updates in this category yet.</p>
                  <button
                    onClick={() => setActiveCategory('all')}
            ...
            <div className="mt-10 -ml-8 rounded-2xl bg-gradient-to-br from-[#040714] to-[#0D1640] border border-white/10 px-8 py-8 text-center">
              <p className="text-xs font-bold tracking-[0.2em] uppercase text-amber-400 mb-2">Never miss an update</p>
              <h3 className="text-xl font-extrabold text-white mb-2">Get updates in your inbox</h3>
              <p className="text-[#9BAEC6] text-sm max-w-sm mx-auto mb-5">
                Subscribe to KWIN City updates for milestone announcements, new data, and portal launches.
              </p>
