...
export default function SourceReferences({
  ...
  return (
    <section
      className={`rounded-2xl border border-gray-200 bg-white/85 ${compact ? "p-3.5" : "p-5"}`}
      aria-label={heading}
    >
          ...
          {sources.map((source) => (
            ...
            >
              <div className="mb-1.5 flex flex-wrap items-center gap-2">
                ...
                <span
                  className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${statusStyles[source.status]}`}
                >
              ...
              <p className="mb-0 text-sm leading-6 text-gray-700">
                {source.note}
              </p>
              {source.url ? (
                <a
